...
create table tb_movie_scores (movie_id int8 not null, scores_movie_id int8 not null, scores_user_id int8 not null, primary key (movie_id, scores_movie_id, scores_user_id));
...
alter table if exists tb_movie_scores add constraint UK_qxodynvtb18nnggofwml1flwu unique (scores_movie_id, scores_user_id);
alter table if exists tb_movie_scores add constraint FKi117ejr8m1jujhfbheai5wv9p foreign key (scores_movie_id, scores_user_id) references tb_score;
alter table if exists tb_movie_scores add constraint FK3eci07b4ikl6ja9j92vvhcr9j foreign key (movie_id) references tb_movie;
